...
export default function HomePage() {
  ...
  const goToArticle = (article: Article) => {
    const params = new URLSearchParams({
      title: article.title || '', description: (article.description || '').slice(0, 500),
      url: article.url || '', image: article.urlToImage || '',
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8" style={{ position: 'relative', zIndex: 1 }}>
            ...
            {hero && (
              ...
              >
                ...
                <div className="absolute inset-0" style={{
                  background: 'linear-gradient(to top, rgba(4,4,15,0.98) 0%, rgba(4,4,15,0.6) 50%, transparent 100%)'
                }} />
                ...
                <div className="absolute bottom-0 left-0 right-0 p-8">
                  ...
                  <h2 className="font-serif text-2xl md:text-3xl font-bold text-white leading-tight mb-3 group-hover:text-accent-purple transition-colors">
                    {hero.title}
                  </h2>
                  <p className="text-white/60 text-sm line-clamp-2">{hero.description}</p>
                </div>
              </div>
            )}
      ...
      {compareList.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-5 py-3.5 rounded-2xl border border-accent-cyan/30"
          style={{
